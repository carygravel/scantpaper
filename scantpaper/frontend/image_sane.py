...
def find_scan_options(
        _class,             started_callback, running_callback,
...
def scan_page_finished_callback( status, path, n_scanned, options ) :
    ...
    if  "new_page_callback"  in options        and not _self["abort_scan"]        and ( status == "STATUS_GOOD" or status == "STATUS_EOF" )     :
        options["new_page_callback"]( status, path, options["start"] )


    # Stop the process unless everything OK and more scans required

    if _self["abort_scan"]        or ( options["npages"] and n_scanned >= options["npages"] )        or (    status != "STATUS_GOOD"
            and status != "STATUS_EOF" )     :
        if _self["abort_scan"] :
            os.remove(path) 
        ...
        if data["type"] == 'finished' :
            ...
            if  "finished"  in callback[ data["uuid"] ] :
                if data["process"] == 'set-option' :
                    ...
                    callback[ data["uuid"] ]["finished"]                       ( _thaw_deref( data["info"] ), data["status"] )
        ...
        while True:
            ...
            if status != "STATUS_GOOD" :
                if parm["depth"] == _8_BIT :
                    logger.info(
                        f"{prog_name}: min/max graylevel value = %d/%d" % (MAXVAL_8_BIT,0) 
                          )
...
def cleanup(parm, total_bytes):
    ...
    if total_bytes > expected_bytes and expected_bytes != 0 :
        logger.info(
            '%s: WARNING: read more data than announced by backend '+'(%u/%u)' % (prog_name,total_bytes,expected_bytes) 
...
def _thread_scan_page( self, uuid, path ) :
    ...
    try :
        ...
        _thread_throw_error( self, uuid, 'scan-page', status,
            f"{prog_name}: sane_start: " + _.error() )
        os.remove(path) 
...
def _log_frame_info( first_frame, parm, format_name ) :
    ...
    if first_frame :
        if parm["lines"] >= 0 :
            logger.info(
                f"{prog_name}: scanning image of size %dx%d pixels at "+'%d bits/pixel' % (parm["pixels_per_line"],parm["lines"],_8_BIT*parm["bytes_per_line"]/parm["pixels_per_line"]) 
            ...
            logger.info(
                f"{prog_name}: scanning image %d pixels wide and "+'variable height at %d bits/pixel' % (parm["pixels_per_line"],_8_BIT*parm["bytes_per_line"]/parm["pixels_per_line"]) 
        ...
        logger.info(
            f"{prog_name}: acquiring %s frame" % (format_name[ parm["format"] ] if parm["format"]<="FRAME_BLUE"else'Unknown') 
...
def _initialise_scan( fh, first_frame, parm ) :
    ...
    if first_frame :
        if parm["format"] == "FRAME_RED"            or parm["format"] == "FRAME_GREEN"            or parm["format"] == "FRAME_BLUE"         :
            if parm["depth"] != _8_BIT :
...
"""    
    return        1 if (
             parm["format"] == "FRAME_RGB"
          or parm["format"] == "FRAME_GRAY"
      )        else 3    ## no critic (ProhibitMagicNumbers)
