...
# selecting single-sided again should also select facing page.
    ...
    assert dialog.num_pages== 2,   'with no source, num-pages not affected by allow-batch-flatbed'
    ...
    options["page"]              = 5
    def anonymous_01():
        assert dialog.page_number_start== 3,       'adding pages should update page-number-start'
...
# 11, 9, 7, 5, 3, 1, -1, -3, -5, -7
    ...
    dialog.num_pages=0
    dialog.side_to_scan='reverse'
    assert dialog.num_pages== 3,   'selecting reverse should automatically limit the number of pages to scan'
    assert dialog.max_pages== 3, 'selecting reverse should automatically limit the max number of pages to scan'
