...
def test_cancel(mock_thread):
    ...
    with (
        patch("scantpaper.basedocument.slurp", return_value="12345"),
...
def test_add_page_in_order_append_skips_renumber():
    ...
    with patch("scantpaper.basedocument.BaseDocument.renumber") as mock_renumber:
        slist.add_page(2, None, 102)
...
def test_cancel_with_pid_1(mock_thread):
    ...
    with (
        patch("scantpaper.basedocument.slurp", return_value=1),
        patch("os.killpg") as mock_killpg,
    ):
        slist.cancel(MagicMock())
...
def test_cancel_with_empty_pid(mock_thread):
    ...
    with (
        patch("scantpaper.basedocument.slurp", return_value=""),
        patch("os.killpg") as mock_killpg,
    ):
        slist.cancel(MagicMock())
        mock_killpg.assert_not_called()
...
def test_cancel_already_dead_pid(mock_thread):
    ...
    with (
        patch("scantpaper.basedocument.slurp", return_value="99999"),
