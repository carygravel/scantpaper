...
def test_edit_profile_dialog(mocker, available_scan_options):
    ...
    patch_dialog = mocker.patch(
        "scantpaper.dialog.scan.Gtk.Dialog", return_value=dialog_mock
    )
...
def test_save_profile_overwrite_dialog(mocker):
    ...
    mocker.patch("scantpaper.dialog.scan.Gtk.Entry", return_value=entry_mock)
    mocker.patch("scantpaper.dialog.scan.Gtk.Box", return_value=MagicMock(spec=Gtk.Box))
    mocker.patch(
        "scantpaper.dialog.scan.Gtk.Label", return_value=MagicMock(spec=Gtk.Label)
    )
    # i18n is not initialised, so we have to mock this
    mocker.patch("scantpaper.dialog.scan._", side_effect=lambda s: s)
    ...
    dialog_mock = mocker.patch(
        "scantpaper.dialog.scan.Gtk.Dialog", side_effect=dialog_side_effect
    )
...
def test_edit_paper_cancel(mocker):
    ...
    patched_dialog_class = mocker.patch(
        "scantpaper.dialog.scan.Dialog", return_value=mock_editor_window
    )
    ...
    mocker.patch(
        "scantpaper.dialog.scan.Gtk.Image.new_from_icon_name",
        return_value=MagicMock(spec=Gtk.Image),
    )
    mocker.patch("scantpaper.dialog.scan.Gtk.Button", side_effect=mock_button_factory)
    mocker.patch(
        "scantpaper.dialog.scan.Gtk.Button.new_with_label",
        side_effect=mock_button_factory,
    )
    ...
    patched_paperlist_class = mocker.patch(
        "scantpaper.dialog.scan.PaperList", return_value=mock_slist
    )
