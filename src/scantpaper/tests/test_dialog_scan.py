...
def test_show(mocker):
    ...
    mocker.patch("scantpaper.dialog.scan.PageControls.show")
...
def test_edit_paper_apply(mocker):
    ...
    # Mock Dialog
    mock_dialog_cls = mocker.patch("scantpaper.dialog.scan.Dialog")
    ...
    mock_show_message_dialog = mocker.patch(
        "scantpaper.dialog.scan.show_message_dialog"
    )

    # Patch Gtk in dialog.scan to avoid TypeErrors when mixing real/mock widgets
    mock_gtk = mocker.patch("scantpaper.dialog.scan.Gtk")
...
def test_cursor_setter_with_window(mocker):
    ...
    mock_gdk = mocker.patch("scantpaper.dialog.scan.Gdk")
...
def test_device_dropdown_changed_garbage_collection(mocker):
    ...
    mocker.patch("scantpaper.dialog.scan.weakref.ref", return_value=lambda: None)
...
def test_race_condition_device_switching(sane_scan_dialog, mainloop_with_timeout):
    ...
    with (
        patch("scantpaper.frontend.image_sane.sane.open") as mock_sane_open,
...
def test_update_options_error_return(mocker):
    ...
    mocker.patch("scantpaper.dialog.scan.copy", side_effect=lambda x: x)
