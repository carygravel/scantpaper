...
The Canvas SHALL support an offset property that controls which portion of the scene is visible, clamped to prevent viewing outside the image bounds.

#### Scenario: Offset set within bounds
- **WHEN** the pixbuf size is (800, 1000)
- **AND** `offset` is set to (100, 200)
- **THEN** the canvas SHALL scroll so that the scene starts at (100, 200) in image coordinates
