# Capability: scan-option-values

Numeric values shown against scan options in the scan dialog SHALL use the
user's locale decimal separator, and numeric values typed by the user SHALL
be interpreted using that same separator, while the SANE backend always
receives canonical numbers.
