...
- [x] 1.1 Add a truncated-PNM import test to
  `scantpaper/tests/test_1631_import_images.py`, alongside `test_import_ppm`:
  build the `rose:` PPM, truncate it to 1000 bytes (as
  `test_101_document.py:861-872` did), import it via
  `slist.import_files(paths=[...])` using the
  `temp_db`/`temp_ppm`/`get_page_sync` fixtures, and assert the resulting page
  is full size (`page.image_object.size == (70, 46)` and mode `"RGB"`). Verify
  the new test passes: `python3 -m pytest
  scantpaper/tests/test_1631_import_images.py -q -p no:cacheprovider`.
